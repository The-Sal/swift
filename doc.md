...
# Day 3 (which is actually 1 months from day 2 I've had more important things to do)
I've discovered my saviour... Someone in swift dev team made by far the most ATTRACTIVE SYMBOL KNOWN TO MAN kind
i give you the `SWIFT_RUNTIME_STDLIB_API` a magicical CLEAN NICE BEUTIFUL FUCKING REPLACEMENT for extern c and all that
janky fuckery. Just one macro (are they called macros idek?) that goes before void in the function declaration AND BANG
ITS NOT PART OF the stdlib well not really BUT it can be found in the swift stdlib via @silgen orwhatever the fuck that thing
was called which is 190000000000x better than module maps or fucking header files which i still dont get why header files exist
but this just cemented my poszition that header files are stupid fucking things. NOW th @siliwhatever the fuck can just snipe out
whatever cpp code i want and the ^^^ macrowhatever thing just makes so it exports and files compiled by it just fucking work this
IS FUCKING COOL OMG I LOVE THISIARgIERSGSEgh. That just means I can go deep in cpp land add whatever the fuck i want and just bring
it back with the above thing WOWWOWWWW. with this magical power in like 2s playAudio works now dyk how insane that is. Like wow do you understand the possiblities this opens for me. Like I did the playAudio thing in the first 2 days and it would compile file but the consumer code? thats being compiled by the custom compiler which compiled fine would not be able to find it for some reason and it would just break but the magic macro just made everything vanish YAY.

Also another small note wtf are compilers anyways and I mean that seriously nothing to do with this but I was working on another swift project that uses SE (secure enclave) and it had a switch statement for the possible ways to auth and one of them was like .touchID sensor or whatever right and it compiled fine on my M4 but then I tried to compile it on an intel machine I had and it went `yea buddy theres no touchID sensor in the security module the fuck you talking about` and I was like huh makes sense if the code RETURNS touch id not available but this time its saying that .touchID doesnt EXIST which is CRAZY because this intel box is with the most recent macOS yes it was built long ago but its running the latest OS so it should know yk.... but then here's the funny part i was liek hmm  the code is just being annoyed about that one thing what if i just cross-compile from my M4 then toss it to the intel will that still work even though inside its world the case value touchID does not exist? and guess what IT FUCKING DID HOW IDEDK but like fuck me this is crazy. what even is code at this point

also also yk one of my other other projects that im working on which is actually more important that this i started this thing were like we have docs/ module which is just docs for all the modules of that project but we also have a docs/TRIAGE and docs/PLATFORM which are directories for like bugs I found and why and how we solved them just bc their interesting. So im doing a similar thing here with a memories/ folder which is like how for example i got playAudio.cpp to work and all the files it touched the theory and magic learnt here bc im fo-sho forgetting what the fuck this is all about in a few days so its like lessons learned i guess and more about the actual swift compiler project and how it works and what not all in that memories/ folder also in the 1 month i was gone litreally 4 swifts were released like wtf are these guys releasing code like rabbits breeding there was a release week on week guys take a god dam fucking nap jeez

Also some additonal learnings I've swtiched to Zed (the editor) instead of Clion because few things compiler building is weird because well the LSP assume the language is made but your making the language it doenst know exists yet and even the C++ is a bit suspicious. Now theres a nice thing with Zed is its minimalistic enough that when it says errors (which CLion does too) its not annoying about it just a small squiggly line that you can ignore because it doesnt matter also the tasks.json is super convient bc the build script is a bit interesting to say the least. Also like most of the time your doing ur own BRAIN-LSP server less what the IDE providers just the life here. One annoying thing though is with Zed the C++ is sub-par relative to CLion and configuring Zed is weird. That being said I am new to zed and ive decided to learn it so i could just be shit. Zed's C++ is weord becaise its saying thiings like filesystem isnt real which is weird when Clion just knows where it is. I presume theres some configuration within settings.json or keymap.json or keymap.default.json.fuckyou.more.json or whatever but CBA rn ill cross that bridge when I get there. So where does that leave me technically it leaves me with both Zed and Clion honestly. Because writing C++ code is SO NICE ON CLion no one is even close but for swift I like Zed more it just shuts up less so I have both IDEs open but Zed is now more like the generalist where i browse around and CLion the specilalist although if i had to die and pick one it's still CLion because its c++ support is stupidly good and Im a jetbrains kid through and through.
